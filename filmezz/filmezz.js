...
(function(plugin) {
    ...
    function trim(s) {
        ...
        return s.replace(re, "").replace(/(^\s*)|(\s*$)/gi, "").replace(/(\r\n|\n|\r)/gm, "").replace(/[ ]{2,}/gi, " ").replace(/\t/g, ' ').replace(/ <br>/g, ', ').replace(/<br>/g, '');
    }
    ...
    plugin.addURI(getDescriptor().id + ":play:(.*):(.*):(.*)", function(page, hoster, url, title) {
        ...
        switch (unescape(hoster)) {
            ...
            case 'Youwatch':
                ...
                break;
            case 'CloudZilla':
                url = showtime.httpReq(doc.headers.Location).toString().match(/<div id="player_container">[\S\s]*?src="([\S\s]*?)"/)[1];
            ...
            case 'Played.To':
                ...
                break;
            case 'Indavideo':
                url = showtime.httpReq(doc.headers.Location).toString();
                url = showtime.httpReq(url.match(/video_src" href="([\S\s]*?)"/)[1]).toString();
                url = url.match(/hash = "([\S\s]*?)"/)[1];
                url = showtime.httpReq('http://amfphp.indavideo.hu/SYm0json.php/player.playerHandler.getVideoData/'+url);
        ...
        function appendItem(route, title) {
            page.appendItem(route, 'video', {
                ...
                description: new showtime.RichText((orig_name ? coloredStr('Eredeti cím: ', orange) + trim(orig_name[1]) + '\n' : '') +
                    ...
                    (actors ? coloredStr('Szereplők: ', orange) + trim(actors[1]) + '\n' : '') +
                    coloredStr('Leírás: ', orange) + trim(desc))
            });
        }
